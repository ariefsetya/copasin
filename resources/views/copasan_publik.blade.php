...
@section('content')
	...
	<?php
		...
		<tr title="{{$key->spam==1?"Dilaporkan sebagai spam":""}}">
			...
			<td style="{{$key->spam==1?"background:red !important":""}}">@if($key->spam==0) <a href="<?php echo url($key->hash);?>"> @endif {!!$key->spam==1?"<s>":""!!}<?php if($key->spam==1){echo substr($key->judul,0,strlen($key->judul)-2)."xx";}else{echo $key->judul;}?>{!!$key->spam==1?"</s>":""!!} @if($key->spam==0)</a>@endif</td>
