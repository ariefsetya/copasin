...
<span><?php echo "oleh ".($user)." / tanggal ".date_format(date_create($data->created_at),"d F Y")." / jam ".date_format(date_create($data->created_at),"H:i:s")." / syntax ".$lang->name." / kadaluarsa ".$exp." / <a href='".url("embed/".$data->hash)."'>embed</a>";?>@if(Auth::check()) @if(\App\Copas::where('hash',$data->hash)->first()['idpengguna']==Auth::user()->id) / <a href="{{url($data->hash.'/edit')}}">ubah</a> / <a href="{{url($data->hash.'/hapus')}}">hapus</a> @endif @endif <?php echo" / ".($data->spam==0?"<a href='".url("lapor/".$data->hash)."'>lapor spam</a> <span style='cursor:pointer' title='Kamu harus masuk terlebih dahulu agar bisa melaporkan copasan ini sebagai spam ^_^'>(?)</span>":"Dilaporkan sebagai spam").(Auth::check()?((Auth::user()->id==1 and $data->spam_fix==0)?" / "."<a href='".url("lapor/".$data->hash)."'>spam fix</a>":""):"");?></span>
